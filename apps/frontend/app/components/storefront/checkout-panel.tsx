...
export function CheckoutPanel({
  ...
  const requiresVerifiedEmail = Boolean(storefrontUser && !storefrontUser.emailVerified)
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {requiresVerifiedEmail && (
        <p className="mb-4 rounded border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
          ...
          <Link href={`/login?mode=verify&email=${encodeURIComponent(storefrontUser.email)}`} className="font-medium underline">
